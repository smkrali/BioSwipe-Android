...
public class SetPatternActivity extends BasePatternActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        messageText.setText("To get started, let's draw your password");
        Log.v("Debug1", "locked the hard home key button");
...
//        homeKeyLoader.lock(this);
    ...
    private void onLeftButtonClicked(boolean close) {
        Log.v("Debug", "setpatternactivity left clicked");
        if (!close) {
            pattern = null;
            updateStage(Stage.Draw);
        } else {
            setResult(RESULT_CANCELED);
            PatternLockUtils.finished = true;
            finish();
            Log.v("Debug1", "finishing set activity pattern");
        }
    }
    ...
    protected void onSetPattern(List<PatternView.Cell> pattern) {
        Log.v("Debug1", "onset pattern called");
